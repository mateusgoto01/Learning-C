...
int main(void){
    ...
    while(1){ 
        ...
        if(strcmp(cmd, "criar") == 0){
            ...
            if(remove != NULL){
                tree_remove(t, remove);
            }
            
        }
        else if(strcmp(cmd, "info") == 0){
            int nodes = tree_count_nodes(t->root);
            int leafs = tree_count_leafs(t->root);
        ...
        else if(strcmp(cmd, "avl?") == 0){
            int avl = tree_avl_check(t->root);
            if(avl == 0)
                printf("nao\n");
            else
                printf("sim\n");
        }
        else if(strcmp(cmd, "caminho-mais-longo") == 0){
                if(t->root == NULL){
                    printf("arvore vazia\n");
                }
                else{
